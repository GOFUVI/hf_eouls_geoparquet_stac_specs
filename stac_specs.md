...
-   **Assets**: Each radar measurement interval produces multiple GeoParquet files as data assets. One interval yields two files for the **radial metrics** data: one containing the observations associated with the negative Bragg peak and another for the positive Bragg peak (the CODAR spectra exhibit two first-order peaks that we process separately). The partition key `pos_bragg` is therefore a binary flag where `0` means “this record belongs to the negative Bragg peak shard” and `1` means “this record belongs to the positive Bragg peak shard.” In addition, each interval produces a small **header** file (containing global metadata from the original file) and a **range info** file (containing summary statistics by range cell). We store these Parquet files in three dedicated subdirectories under each station: `radial_metrics/`, `header/`, and `rng_info/`. This organization by data type keeps all Parquet files for a station grouped and also allows partitioning by attributes (for example, within `radial_metrics/` we further partition by `pos_bragg=0` vs `pos_bragg=1`, and by timestamp). Using a consistent folder structure makes it easy to run queries across all files of a given type for a station (e.g., scanning all radial metrics files in a date range).
...
\<!\-- \--\>

    {
      ...
      "sci:doi": "10.5281/zenodo.16892222",
      "sci:citation": "Herrera Cortijo, J. L., Fernández-Baladrón, A., Rosón, G., Gil Coto, M., Dubert, J., & Varela Benvenuto, R. (2025). Project HF-EOLUS. Task 2. VILA & PRIO Radial Metrics GeoParquet Dataset. Derived from LLUV datasets and ingested in GeoParquet format. Zenodo. https://doi.org/10.5281/zenodo.16892222",
      "providers": [
        {
...
The radial-metrics archive (Zenodo DOI 10.5281/zenodo.16892222) is the authoritative STAC root for the VILA and PRIO CODAR stations. A top-level `catalog.json` exposes child catalogs per station, and those child catalogs publish rolling Collections that cover uninterrupted acquisition windows so that long campaigns can be browsed without downloading the entire archive. Collections always advertise the LLUV processing parameters, antenna patterns, and deployment metadata through custom `hf_site:*` and `radial_metrics:*` properties so downstream users can reapply the same filters when cross-referencing other HF-EOLUS catalogs.

Every timestamp materializes three sibling Items—`radial_metrics`, `header`, and `rng_info`—that collectively describe the echoes, the LLUV metadata key/value pairs, and the range diagnostics. The Items share the same `datetime`, bounding box, and Collection identifier, but differentiate themselves through their `id` prefix. The radial Item exposes two assets (`neg_bragg_peak` and `pos_bragg_peak`) pointing to the Parquet shards stored under `radial_metrics/pos_bragg={0|1}/timestamp=<slot>/...parquet`; the `pos_bragg` partition key is a binary flag where `0` refers to the negative peak shard and `1` to the positive one. Header and range Items publish a single asset (`header` or `rng_info`) pointing to their respective Parquet tables. Cross-links keep the trio connected: the radial Item uses `rel:describedby` and `rel:related` links to point at the header and range Items for the same timestamp, while the header Item responds with `rel:describes` so clients can navigate in both directions. All Items adopt the STAC Table Extension v1.2.0 to list the tabular schema, set `table:primary_geometry`, and expose the row counts baked into the GeoParquet footer, ensuring that catalog-only inspection already conveys column names such as `pos_bragg`, `RNGE`, or `NVAC`. Because the catalog is distributed as a compressed archive, the relative layout (`hf_ingestion/VILA/radial_metrics/...`) matches the folder structure documented in the GeoParquet inventory and can be mirrored directly into an object store.

### Sentinel-1 SAR ingestion

The Sentinel-1 catalog (Zenodo DOI 10.5281/zenodo.17007304) is flatter because it represents a single Collection named after the ingestion window. Each daily Item corresponds to one `date=YYYY-MM-DD` GeoParquet partition emitted by the ingestion pipeline, and the Item geometry matches the convex hull of all OWI vectors produced on that day. Items expose a single `data` asset (`../assets/date=<day>/<day>.parquet`) with `roles: ["data"]`, so schema and provenance details are conveyed entirely through Item-level metadata rather than through external sidecars.

Each Item activates the STAC Table Extension to advertise the OWI vector schema (`geometry`, `owiWindSpeed`, `owiWindDirection`, ESA quality flags, and the `rowid` hash used downstream) and the STAC Processing Extension to cite the upstream Sentinel-1 product identifiers captured in the `processing:lineage` property. Because the same Collection covers the full time span, Items use the `datetime` attribute for midpoint timestamps and set `start_datetime`/`end_datetime` to the exact `firstMeasurementTime` and `lastMeasurementTime` captured in the Parquet metadata. The catalog’s `providers` block and `sci:doi` fields match the DOIs declared in `AGENTS.md`, which keeps Zenodo, ESA, and HF-EOLUS acknowledgements synchronized across every document.

### Puertos del Estado buoy ingestion

The PdE buoy ingestion catalog (Zenodo DOI 10.5281/zenodo.17098037) organizes Collections per buoy so that Vilano, Prior, or any future reference stations remain isolated archives. Each Collection covers the full time extent currently delivered for that buoy and exposes the Scientific Extension metadata (`sci:doi`, `sci:citation`, license, and provider statements) that cite Puertos del Estado as the original source. Items follow the native acquisition cadence—hourly for Vilano—and reuse the buoy point geometry as both Item geometry and Asset primary geometry so spatial consumers can filter by the exact buoy location.

Per-item assets are intentionally simple: each Item exposes a single GeoParquet file (`assets/<buoy>.parquet`) containing the calibrated wind series, and provenance context (Athena CTAS SQL, ingestion logs) is distributed alongside the repository but not embedded as STAC sidecars. Items declare the Table Extension to list `timestamp`, `wind_speed`, `wind_dir`, and `geometry`, and reuse the Scientific Extension fields inherited from the Collection so that every hourly record can be cited independently.

### HF-EOLUS geo tools aggregation

The geo-tools catalog (Zenodo DOI 10.5281/zenodo.17105305) ships three independent catalog trees—`vila_catalog`, `prio_catalog`, and `sar_catalog`—because the aggregated HF and SAR products are curated separately. Each HF catalog nests an extra level per Bragg peak (`items/pos_bragg_0` and `items/pos_bragg_1`), and every leaf Collection hosts a single Item that references the consolidated Parquet file for that peak (for example `assets/pos_bragg=1/1.parquet`). SAR aggregation Items live under `sar_catalog/items/` and expose consolidated assets (no partitions) because the OWI grid is already aligned with the HF mesh.

The Items are intentionally minimal: each one carries a single `data` asset and only the standard STAC navigation links (`root`, `parent`, `collection`). There are no helper assets or `derived_from` links in the published catalogs; provenance back to the radial metrics catalog is handled through the documentation and the shared `hf_site:*` metadata fields rather than through link relations. The Table Extension lists the aggregated metrics (`pwr_*`, `velo_*`, SAR statistics) and identifies `node_id`/`geometry` as the primary keys, while custom properties capture antenna footprints, aggregation window sizes, and CRS hints so consumers understand how radar and SAR contributions coexist in each partition.

### HF-Radar wind inversion toolkit

The wind-inversion toolkit (Zenodo DOI 10.5281/zenodo.17131227) exposes a root catalog with sub-catalogs for each experimental branch: `data_preparation_pipeline`, `grid_offset_pipeline`, `vilano_pipeline`, `sar_pipeline`, and hybrid SAR+Vilano releases. Collections inside those branches mirror the ANN lifecycle (training folds, inference checkpoints, range-calibration experiments) so analysts can navigate by scenario rather than by station. Items bundle complete GeoParquet snapshots—always stored as `assets/data.parquet`—that correspond to either pivoted training tables or inference outputs, while the `properties` block advertises the ANN configuration (feature families, target heads, and version tags) used to create that asset.

Published Items contain a single `data` asset plus the standard STAC navigation links; they do not publish `derived_from` or `related` links to the upstream geo-tools or buoy catalogs. Provenance is instead conveyed through the scenario naming and the metadata recorded in `properties`. Table Extension metadata enumerates the spatial keys and prediction columns (`pred_wind_speed`, `prob_range_*`, maintenance diagnostics) so catalog consumers understand the model outputs before downloading the files, and the consistent `assets/data.parquet` naming keeps ingestion scripts straightforward across all sub-catalogs.

### Wind resource toolkit

The wind-resource catalog (Zenodo DOI 10.5281/zenodo.17594220, workflow DOI 10.5281/zenodo.17591545) inherits inference products from the previous workflow and republishes them as resource-estimation Collections. Each Collection maps to a release such as `sar_range_final_pivots_joined` or `sar_range_final_power_estimates`. Items that mirror upstream inference tables (`sar_range_final_pivots_joined`, `pde_vilano_buoy`) expose a single `data` asset, whereas the power-estimate deliverables add a second asset (`manifest`) that points to `manifest.json` with checksum, bootstrap, and code-commit details. Only the power-estimate Item declares `rel:derived_from`, linking back to the pivots Collection that supplied the inputs; the other Items retain the default STAC navigation links.

The Table Extension lists every KPI column included in the resource tables (Weibull parameters, power density statistics, QA flags) along with the spatial keys, while the Scientific Extension now embeds the Zenodo DOI citation block referenced above. Because power-analysis Items summarize long periods, they set `start_datetime`/`end_datetime` to the aggregated window and store the execution date under a custom `hf_eolus:release_date` property. Asset roles distinguish between the GeoParquet payload (`data`) and the optional provenance bundle (`manifest`), making it clear which file holds the evidence used during publication, and both assets inherit the DOI metadata to preserve traceability when mirrored outside Zenodo.

### MeteoGalicia wind interpolation

The MeteoGalicia interpolation catalog (Zenodo DOI 10.5281/zenodo.17490872, workflow DOI 10.5281/zenodo.17598353) is intentionally independent: it has its own root (`wind_interpolation/case_study/catalogs/catalog.json`), is distributed as a standalone Zenodo bundle, and does not publish `derived_from` links to other HF-EOLUS branches because its outputs are not yet ingested by the wind-resource toolkit. Collections are organized by experiment (for example the production sync versus case-study runs) and by model configuration, making it easy to contrast hourly deliverables originating from different MeteoGalicia domains.

Each hourly GeoParquet shard is represented by a data Item with a single `data` asset (`assets/data.parquet`). Companion metadata and plots live in sibling catalogs (`items/metadata`, `items/plots`) whose Items reference the JSON summaries and PNG diagnostics. The three catalogs are interlinked through `rel:describedby` and `rel:related` links so that a consumer landing on the data Item can hop to the metadata/plot Items without embedding those files as extra assets. Table Extension metadata enumerates the spatial descriptors (`node_id`, `geometry`, `x_local`, `y_local`) and the interpolation outputs (`wind_speed`, `wind_direction`, quality metrics), while Collection-level summaries list the set of `source_model` values present in the archive. Because this catalog is decoupled from the ANN and resource lines, it is clearly labeled as a self-contained branch whose assets may be consumed independently or integrated later without renaming.
...
### Radial Metrics Data Columns
  ...
  **pos\_bragg**   Binary flag indicating which Bragg peak generated the record (0 = negative peak shard, 1 = positive peak shard)

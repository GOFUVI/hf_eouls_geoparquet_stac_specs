...
The Sentinel-1 catalog (Zenodo DOI 10.5281/zenodo.17100125) is flatter because it represents a single Collection named after the ingestion window. Each daily Item corresponds to one `date=YYYY-MM-DD` GeoParquet partition emitted by the ingestion pipeline, and the Item geometry matches the convex hull of all OWI vectors produced on that day. Items expose a single `data` asset (`../assets/date=<day>/<day>.parquet`) with `roles: ["data"]`, so schema and provenance details are conveyed entirely through Item-level metadata rather than through external sidecars.
...
The wind-resource catalog (DOI pending publication, see `AGENTS.md`) inherits inference products from the previous workflow and republishes them as resource-estimation Collections. Each Collection maps to a release such as `sar_range_final_pivots_joined` or `sar_range_final_power_estimates`. Items that mirror upstream inference tables (`sar_range_final_pivots_joined`, `pde_vilano_buoy`) expose a single `data` asset, whereas the power-estimate deliverables add a second asset (`manifest`) that points to `manifest.json` with checksum, bootstrap, and code-commit details. Only the power-estimate Item declares `rel:derived_from`, linking back to the pivots Collection that supplied the inputs; the other Items retain the default STAC navigation links.

The Table Extension lists every KPI column included in the resource tables (Weibull parameters, power density statistics, QA flags) along with the spatial keys, while the Scientific Extension captures the publication metadata once the Zenodo DOI is minted. Because power-analysis Items summarize long periods, they set `start_datetime`/`end_datetime` to the aggregated window and store the execution date under a custom `hf_eolus:release_date` property. Asset roles distinguish between the GeoParquet payload (`data`) and the optional provenance bundle (`manifest`), making it clear which file holds the evidence used during publication.

### MeteoGalicia wind interpolation

The MeteoGalicia interpolation catalog is intentionally independent: it has its own root (`wind_interpolation/case_study/catalogs/catalog.json`), is distributed as a standalone Zenodo bundle, and does not publish `derived_from` links to other HF-EOLUS branches because its outputs are not yet ingested by the wind-resource toolkit. Collections are organized by experiment (for example the production sync versus case-study runs) and by model configuration, making it easy to contrast hourly deliverables originating from different MeteoGalicia domains.
